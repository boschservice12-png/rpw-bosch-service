...
ok(fill('prog',{njDate:njDay(1)}).length===0,'prog: rendszam+telefon+datum eleg');
ok(fill('prog',{}).indexOf('nj_m_date')>=0,'prog: datum nelkul blokkol');
ok(fill('dosar',{njPay:'deschid'}).length===0,'dosar: fizetes-allapottal mehet');
ok(fill('dosar',{}).indexOf('nj_m_pay')>=0,'dosar: dosszie-allapot nelkul blokkol');
...
ok(fill('lucrare',{njTip:'asig',njPay:'deschid'}).length===0,'  megadva -> mehet');
...
(async()=>{
  ...
  j=await mk('prog',{njDate:njDay(1)});
  ok(!!j,'[prog] letrejott');
  ...
  j=await mk('dosar',{njPay:'deschis',njDosar:'D-77',njAsig:'Groupama'});
  ...
  j=await mk('lucrare',{njTip:'asig',njPay:'deschid',njAsig:'Allianz Tiriac'});
