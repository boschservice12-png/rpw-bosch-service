...
// Az Avizare dauna gombja a szeleten KIVUL el (deschideDosar + dosarTarziu),
// de ugyanaz a belepesi pont: ide is be kell toltenunk, kulonben a teszt
// olyan kodot mer, amit a felhasznalo nem is lat.
const c=html.indexOf('window.deschideDosar='), d=html.indexOf('window.dosarFisier=');
...
(async()=>{
  ...
  eq(j.inchis,false,'  nincs lezarva'); eq(categorizeJob(j),'dosare','  -> Dosare ful');
  eq(j.plate,'','  rendszam ures — a dosszie lapjan toltik ki');
  eq(j.nrDosar,'','  karszam ures — a dosszie lapjan toltik ki');
